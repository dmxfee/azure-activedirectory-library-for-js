...
AuthenticationContext = function (config) {
    ...
    this.CONSTANTS = {
        ...
        POPUP_WIDTH: 483,
        POPUP_HEIGHT: 600,
        MSA_TID: "f8cdef31-a31e-4b4a-93e4-5f571e91255a"
    };
 ...
 */
AuthenticationContext.prototype.login = function (loginReason) {
    ...
    // need to phase this out by updating to use sid
    if (this._user && this._user.profile) {
        if (!this._isMsaUser(this._user.profile)){
            if (!this._isGuestUser(this._user.profile)) {
    ...
    // Adal telemetry event
    this.event('adal_login', { resource: this.config.clientId, reason: loginReason });
...
AuthenticationContext.prototype._isGuestUser = function (userProfile) {
    // User the parsed json token to identify if this user is a guest
    return (userProfile && userProfile.hasOwnProperty('idp') &&
      userProfile.hasOwnProperty('iss') &&
      userProfile.iss !== userProfile.idp);
}

AuthenticationContext.prototype._isMsaUser = function (userProfile) {
    // parse the json token to identify if this user is MSA user
    return (userProfile && userProfile.hasOwnProperty('altsecid') &&
            userProfile.altsecid.indexOf('1:') === 0);
}
...
AuthenticationContext.prototype._createUser = function (idToken) {
    ...
    if (parsedJson && parsedJson.hasOwnProperty('aud')) {
        ...
        if (parsedJson.aud.toLowerCase() === this.config.clientId.toLowerCase()) {
            ...
            if (parsedJson.hasOwnProperty('upn') && !this._isGuestUser(parsedJson)) {
...
AuthenticationContext.prototype.saveTokenFromHash = function (requestInfo) {
    ...
    if (requestInfo.parameters.hasOwnProperty(this.CONSTANTS.ERROR_DESCRIPTION)) {
        this.error('Error :' + requestInfo.parameters.error + '; Error description:' + requestInfo.parameters[this.CONSTANTS.ERROR_DESCRIPTION]);
...
AuthenticationContext.prototype._getNavigateUrl = function (responseType, resource) {
    var tenant = 'common';
    /**
     * OTP user (first landing: tenant id == MSA_TID) : DO NOT replace "common" at first landing.
     * Other users: replace "common" if tenant id exists.
     */
    if ((this.config.tenant && this.config.tenant != this.CONSTANTS.MSA_TID) || (this._user && this._user.profile && this._isMsaUser(this._user.profile))) {
        tenant = this.config.tenant;
    }
