...
AuthenticationContext.prototype._getNavigateUrl = function (responseType, resource) {
    var tenant = 'common';
    /**
     * OTP user (first landing: tenant id == MICROSOFT_SERVICE_TENANT_ID) : DO NOT replace "/common" at first landing.
     * Other users: replace "/common" if tenant id exists.
     */
    if ((this.config.tenant && this.config.tenant != this.CONSTANTS.MICROSOFT_SERVICE_TENANT_ID) || (this._user && this._user.profile && this._isMsaUser(this._user.profile))) {
        tenant = this.config.tenant;
    }
