        ...
        AdalModule.provider('adalAuthenticationService', function () {
            ...
            this.init = function (configOptions, httpProvider) {
                if (configOptions) {
                    ...
                    if (configOptions.isOffline && typeof configOptions.isOffline !== 'function') {
                        throw new Error('callback isOnline is not a function');
                    }
                    else{
                        _isOffline = configOptions.isOffline;
                    }
            ...
            this.$get = ['$rootScope', '$window', '$q', '$location', '$timeout', '$injector', function ($rootScope, $window, $q, $location, $timeout, $injector) {
                ...
                var locationChangeHandler = function () {
                    ...
                    if (_adal.isCallback(hash)) {
                        ...
                        if (requestInfo.stateMatch) {
                            if (typeof _adal.callback === 'function') {
                                ...
                                if (_oauthData.userName) {
                                    ...
                                    $rootScope.$broadcast('adal:loginSuccess', _adal.config.loginResource);
                                } else {
                                    $rootScope.$broadcast('adal:loginFailure', _adal._getItem(_adal.CONSTANTS.STORAGE.ERROR_DESCRIPTION), _adal.config.loginResource);
                                }
                            }
                        }
                        else {
                        ...
                        //If the app is offline do nothing on route change. This will stop kicking clients out.
                        if(_isOffline && _isOffline($injector)) {
                            _adal.warn('Location change event ignore due to app state offline.');
                            return;
                        }
                        else{
                            updateDataFromCache(_adal.config.loginResource);
                            if (!_oauthData.isAuthenticated && _oauthData.userName && !_adal._renewActive) {
                                ...
                                _adal.acquireToken(_adal.config.loginResource, function (error, tokenOut) {
                                    ...
                                    if (error) {
                                        $rootScope.$broadcast('adal:loginFailure', 'auto renew failure', _adal.config.loginResource);
                                    } else {
                                        if (tokenOut) {
                ...
                var loginHandler = function (loginReason) {
                ...
                var routeChangeHandler = function (e, nextRoute) {
                    if (nextRoute && nextRoute.$$route && isADLoginRequired(nextRoute.$$route, _adal.config)) {
                        if (!_oauthData.isAuthenticated && !_adal._renewActive) {
                            ...
                            loginHandler('adal:route_change&unauthenticated&renew_inactive');
                        }
                    }
                };
                ...
                var stateChangeHandler = function (e, toState, toParams, fromState, fromParams) {
                    if (toState && isADLoginRequired(toState, _adal.config)) {
                        if (!_oauthData.isAuthenticated && !_adal._renewActive) {
                            //TODO: Take a look at this for deep linking..
                            ...
                            loginHandler('adal:state_change&unauthenticated&renew_inactive');
                        }
                    }
                };
                ...
                return {
                    ...
                    acquireToken: function (resource) {
                        ...
                        var isLoginResource = resource === _adal.config.loginResource;
                        //Renew active is only used for login scenarios so only set it when fetching the login resource.
                        _adal._renewActive = isLoginResource ? true : _adal._renewActive;
                        _adal.acquireToken(resource, function (error, tokenOut) {
                            _adal._renewActive = isLoginResource ? false : _adal._renewActive;
                            if (error) {
                                _adal.error('Error when acquiring token for resource: ' + resource, error);
                                deferred.reject(error);
                            } else {
                                isLoginResource && updateDataFromCache(_adal.config.loginResource);
